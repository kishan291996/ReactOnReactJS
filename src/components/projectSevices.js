...
export const addTaskToProject = async (taskData) => { try { const response = await fetch(`${'http://localhost:5000/api'}/addTask`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(taskData) }); if (!response.ok) { throw new Error('Failed to add task'); } return response.json(); } catch (error) { console.error('Error adding task:', error); return { error: 'Error adding task' };}}

export const fetchTasksByProjectId = async (projectId) => { try { const response = await fetch(`${'http://localhost:5000/api'}/tasks?projectId=${projectId}`); if (!response.ok) { throw new Error('Failed to fetch tasks'); } return response.json(); } catch (error) { console.error('Error fetching tasks:', error); return []; }}